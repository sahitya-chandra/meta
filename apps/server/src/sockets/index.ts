...
export const setupSockets = (io: Server) => {
  ...
  io.on('connection', async (socket: AuthenticatedSocket) => {
    ...
    socket.on(
      'sendMessage',
      async (payload: { toUserId: string; content: string }) => {
        ...
        try {
          ...
          console.log(
            `Receiver socket ID for ${toUserId}: ${receiverSocketId}`
          );

          if (receiverSocketId) {
            io.to(receiverSocketId).emit('newMessage', message);
            console.log(
              `Emitted newMessage to ${toUserId} (socket: ${receiverSocketId})`
            );
          } else {
          ...
          console.log(
            `Emitted messageSent to ${senderId} (socket: ${socket.id})`
          );
        } catch (err) {
